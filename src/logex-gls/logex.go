...
func init() {
	...
	if len(os.Getenv("LOG_LEVEL")) > 0 {
		DebugLevel, err = strconv.Atoi(os.Getenv("LOG_LEVEL"))
		if err != nil {
			DebugLevel = 1
		}
	}
...
func (l Logger) makePrefix(calldepth int) string {
	...
	var f string
	var name string
	pc, f, line, ok := runtime.Caller(calldepth - 1)
	if !ok {
		f = "?"
		name = "?"
	} else {
	...
	gls_req_id := glsGet(FORWARDED_FOR, ",") + glsGet(REQUEST_IP_ADDR, ",") + glsGet(RAND_ID, ",") + glsGet(REQUEST_ID, ",") + glsGet(REQUEST_PATH, ",") + glsGet(USER_ID, ",")
